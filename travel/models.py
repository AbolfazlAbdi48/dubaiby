...
class Image(models.Model):
    hotel = models.ForeignKey(Hotel, verbose_name=_('هتل'), related_name='images', on_delete=models.CASCADE, null=True,
                              blank=True)
    # tour = models.ForeignKey('Tour', verbose_name=_('تور'),
    # related_name='images', on_delete=models.CASCADE, null=True, blank=True)
    image = models.ImageField(verbose_name=_('تصویر'), upload_to='images/')

    def __str__(self):
        return f"{self.hotel.title} - {self.image.url}"
...
class HotelRoom(models.Model):
    ...
    def __str__(self):
        return f"{self.hotel.title} - {self.meal_plan} - {self.price_per_night}"
...
class Tour(models.Model):
    ...
    transport_ticket = models.ForeignKey(to='FlightTicket', verbose_name=_('حمل و نقل (بلیط پرواز)'), on_delete=models.SET_NULL,
